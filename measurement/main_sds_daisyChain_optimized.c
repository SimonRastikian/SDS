...
int main (void){
    ...
    for (size_t i=0; i<ITER_NUM; i++){
        ...
        for (size_t j=0; j<CHAIN_LEN; j++){
          start = end = 0;
          start = clock();
          sds_daisyChain_optimized_sot_sig(&sigma, &sst, m[i*ITER_NUM+j]);
          end = clock();
          cpu_time_sig += ((double) (end*1000000 - start*1000000));

          start = end = 0;
          start = clock();
          sds_daisyChain_optimized_sot_ver(&vst, m[i*ITER_NUM+j], sigma);
          end = clock();
    ...
    printf("--------------------SDS DaisyChain for OPTIMIZED SOT------------------------\n");

    printf("Optimized SDS DaisyChain keygen time in microseconds per %d calls for %d messages each: %lf\n", ITER_NUM,CHAIN_LEN,cpu_time_keygen/CLOCKS_PER_SEC);
    printf("Optimized SDS DaisyChain sig time in microseconds per %d calls: %lf\n", ITER_NUM*CHAIN_LEN,cpu_time_sig/CLOCKS_PER_SEC);
    printf("Optimized SDS DaisyChain ver time in microseconds per %d calls: %lf\n", ITER_NUM*CHAIN_LEN,cpu_time_ver/CLOCKS_PER_SEC);
    ...
    printf("Optimized SDS DaisyChain keygen time in microseconds per call: %lf\n", cpu_time_keygen/CLOCKS_PER_SEC);
    printf("Optimized SDS DaisyChain sig time in microseconds per call: %lf\n", cpu_time_sig/CLOCKS_PER_SEC);
    printf("Optimized SDS DaisyChain ver time in microseconds per call: %lf\n", cpu_time_ver/CLOCKS_PER_SEC);
